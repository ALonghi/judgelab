...
function shell(section) {
  ...
  $('#shell').innerHTML=`<aside class="sidebar">
    ...
    <div class="sidebar-bottom"><div class="local-indicator"><i></i>Local Python · no API keys</div><div class="profile"><div class="avatar">AL</div><div><strong>Alessio’s practice lab</strong><small>Backend engineering playground</small></div></div></div>
  </aside>
...
function renderLesson(){
 ...
 $('#main').innerHTML=lessonHeader()+(current.kind==='quiz'?quizView():phaseTabs()+(phase===0?lessonBrief(current):'')+(current.kind==='code'?codeView():discussionView()));
...
function lessonBrief(lesson){
 return strategyView(lesson);
}
function strategyView(lesson){
 const strategy=lesson.strategy;
 const {problem,brief}=lesson;
 if(!strategy||!problem||!brief)return '';
 return `<article class="lesson-narrative">
 <section class="learning-step" data-learning-step="problem" aria-labelledby="problem-heading"><span class="brief-label">01 · THE PROBLEM</span><h2 id="problem-heading">Start with the situation</h2><p>${escapeHTML(problem.problem)}</p><details class="interview-framing"><summary>How this could come up in an interview</summary><blockquote>${escapeHTML(strategy.prompt)}</blockquote><small>Illustrative practice scenario.</small></details></section>
 <section class="learning-step" data-learning-step="constraints" aria-labelledby="constraints-heading"><span class="brief-label">02 · CLARIFY BEFORE CODING</span><h2 id="constraints-heading">What do you need to find out?</h2><div class="learning-vocabulary"><h3>Terms in this problem</h3><dl>${brief.vocabulary.map(item=>`<div><dt>${escapeHTML(item.term)}</dt><dd>${escapeHTML(item.meaning)}</dd></div>`).join('')}</dl></div>${decisionGuide(lesson)}</section>
 <section class="learning-step" data-learning-step="reasoning" aria-labelledby="reasoning-heading"><span class="brief-label">03 · WORK THROUGH IT</span><h2 id="reasoning-heading">Follow a concrete case</h2><ol class="learning-reasoning">${problem.reasoning.map(item=>`<li>${escapeHTML(item)}</li>`).join('')}</ol></section>
 <section class="learning-step" data-learning-step="approach" aria-labelledby="approach-heading"><span class="brief-label">04 · IMPLEMENT THE APPROACH</span><h2 id="approach-heading">${escapeHTML(strategy.name)}</h2>${implementationGuide(lesson)}<details class="strategy-depth"><summary>Where this approach falls short</summary><div class="strategy-detail-body"><ul>${strategy.caveats.map(item=>`<li>${escapeHTML(item)}</li>`).join('')}</ul>${strategy.references?.length?`<div class="strategy-references"><span>Technical reading</span>${strategy.references.map(ref=>`<a href="${escapeHTML(ref.url)}" target="_blank" rel="noopener noreferrer">${escapeHTML(ref.title)} ↗</a>`).join('')}</div>`:''}</div></details></section>
 <section class="learning-step learning-practice" data-learning-step="practice" aria-labelledby="practice-heading"><span class="brief-label">05 · TRY IT YOURSELF</span><h2 id="practice-heading">${lesson.kind==='discussion'?'Prepare your design':'The small version you’ll practise'}</h2><p>${escapeHTML(brief.rule)}</p>${lesson.id==='q-context'?'<p class="practice-scope">The example above used tokens to reflect a model request. The exercise below uses word counts so you can practise the selection logic without a tokenizer.</p>':''}</section>
 </article>`;
}
function decisionGuide(lesson){
 const guide=lesson.implementation;
 if(!guide)return '';
 return `<div class="decision-guide">${guide.decisions.map(decision=>`<section class="decision-question">${decision.lead_in?`<p class="decision-context">${escapeHTML(decision.lead_in)}</p>`:''}<h3>${escapeHTML(decision.question)}</h3><dl>${decision.branches.map(branch=>`<div><dt>${escapeHTML(branch.answer)}</dt><dd>${escapeHTML(branch.action)}</dd></div>`).join('')}</dl></section>`).join('')}${guide.ranking_code?`<h3>A minimal ranking example</h3><pre class="teaching-code" tabindex="0" aria-label="Python ranking example"><code>${escapeHTML(guide.ranking_code)}</code></pre><p>${escapeHTML(guide.ranking_note)}</p>${guide.ranking_reference?`<p class="strategy-references"><a href="${escapeHTML(guide.ranking_reference.url)}" target="_blank" rel="noopener noreferrer">${escapeHTML(guide.ranking_reference.title)} ↗</a></p>`:''}`:''}${guide.connections?.length?`<nav class="learning-connections" aria-label="Related lessons"><h3>Where this fits in the learning path</h3><ul>${guide.connections.map(link=>`<li><a href="#lesson/${escapeHTML(link.id)}" data-action="lesson" data-id="${escapeHTML(link.id)}">${escapeHTML(findLesson(link.id).title)}</a><p>${escapeHTML(link.text)}</p></li>`).join('')}</ul></nav>`:''}</div>`;
}
function implementationGuide(lesson){
 const guide=lesson.implementation;
 if(!guide)return '';
 return `<div class="implementation-guide"><h3>${lesson.kind==='discussion'?'State and responsibilities':'What to keep track of'}</h3><p>${escapeHTML(guide.state)}</p><h3>${lesson.kind==='discussion'?'Concrete operations and failure handling':'What to do, in order'}</h3><ol class="learning-reasoning">${guide.steps.map(step=>`<li>${escapeHTML(step)}</li>`).join('')}</ol>${guide.code?`<h3>Python example</h3><pre class="teaching-code" tabindex="0" aria-label="Python selection example"><code>${escapeHTML(guide.code)}</code></pre>`:''}${guide.trace?`<div class="teaching-trace" role="region" aria-label="${escapeHTML(guide.trace.caption||'Budget trace')}" tabindex="0"><table><caption>${escapeHTML(guide.trace.caption||'Track the budget through the loop (tokens)')}</caption><thead><tr>${guide.trace.headers.map(header=>`<th scope="col">${escapeHTML(header)}</th>`).join('')}</tr></thead><tbody>${guide.trace.rows.map(row=>`<tr>${row.map(cell=>`<td>${escapeHTML(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`:''}<h3>Result to verify</h3><p>${escapeHTML(guide.result)}</p></div>`;
}
function quizView(){
 return `<div class="quiz-lesson">${lessonBrief(current)}${quizQuestionView(quizResult)}</div>`;
}
