...
function workedExample(lesson) {
 ...
 return `<figure class="scoring-example"><figcaption>Follow the matches</figcaption>
...
function productionDecision(lesson){
 ...
 if(!decision)return '';
 return `<section class="decision-introduction"><p>${escapeHTML(decision.lead_in)}</p><details class="design-decision"><summary>${escapeHTML(decision.question)}</summary><dl>${decision.branches.map(branch=>`<div><dt>${escapeHTML(branch.answer)}</dt><dd>${escapeHTML(branch.action)}</dd></div>`).join('')}</dl></details></section>`;
}
function productionContext(lesson){
 const strategy=lesson.strategy;
 return `<details class="production-context"><summary>How would this change in a real service?</summary><div class="production-explanation"><h3>How to put the approach into practice</h3><p>${escapeHTML(strategy.proposal)}</p></div><div class="production-considerations">${productionDecision(lesson)}<h3>When would you choose differently?</h3><ul class="reference-list">${strategy.caveats.map(item=>`<li>${escapeHTML(item)}</li>`).join('')}</ul></div></details>`;
}
function lessonOrientation(lesson){
 const guide=lesson.orientation;
 if(!guide)return '';
 return `<section class="lesson-orientation" aria-label="Understand the task"><h2>What problem are we solving?</h2><p>${escapeHTML(lesson.strategy.use_case)}</p><h3>The idea</h3><p>${escapeHTML(lesson.strategy.mechanism)}</p><dl class="lesson-handoff"><div><dt>What you start with</dt><dd>${escapeHTML(guide.input)}</dd></div><div><dt>${lesson.kind==='discussion'?'What you design':lesson.kind==='quiz'?'What you decide':'What you build'}</dt><dd>${escapeHTML(lesson.scenario.deliverable)}</dd></div><div><dt>What comes out</dt><dd>${escapeHTML(guide.output)}</dd></div></dl><p class="lesson-scope">${escapeHTML(guide.scope)}</p></section>`;
}
function schemaExample(lesson){
 const indexed=lesson.pack==='indexed';
 if(!indexed&&lesson.id!=='c-fts'&&lesson.id!=='u-extract')return '';
 const table=(name,description,headers,rows)=>`<div class="schema-table"><p>${escapeHTML(description)}</p><div class="teaching-trace" tabindex="0" role="region" aria-label="${escapeHTML(name)} example rows"><table><caption>${escapeHTML(name)}</caption><thead><tr>${headers.map(h=>`<th scope="col">${escapeHTML(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(row=>`<tr>${row.map(cell=>`<td>${escapeHTML(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div></div>`;
 if(lesson.id==='u-extract')return `<section class="schema-example"><h2>Where the extracted lines go</h2><p>The supplied index_revision() adapter consumes your lines and updates these tables together in one transaction. You implement extraction, while this adapter handles the database writes.</p>${table('revisions','One accepted version per document, retained even after deletion.',['document_id','version'],[['d1','2']])}<p class="schema-link">revisions.document_id = d1 → sections.document_id = d1</p>${table('sections · FTS5 virtual table','Each nonempty line becomes a section of the accepted revision. Spaces and carriage returns are preserved, including whitespace-only lines.',['document_id','version','section_id','body'],[['d1','2','0','Lease notice'],['d1','2','1','Notice period']])}<p>If later decoding fails, the transaction rolls back and the earlier searchable revision remains. This local demo assumes one tenant and a trusted caller; these tables do not enforce user permissions.</p></section>`;
 if(!indexed)return `<section class="schema-example"><h2>See the tables search reads</h2><p>This database belongs to one customer. The supplied setup inserts articles; SQLite maintains the full-text index automatically.</p>${table('articles · FTS5 virtual table','One searchable article. Its generated rowid connects it to its access grants.',['rowid','document_id','title','body','public'],[['7','d1','Reset password','Use the reset link','0']])}<p class="schema-link">articles.rowid = 7 → grants.article_rowid = 7</p>${table('grants','One row per user allowed to read a private article.',['article_rowid','user_id'],[['7','reader-a']])}<p>MATCH reads the FTS5 word index; rank is computed for the query. Neither is a column you populate yourself. There is no application postings table in this pack.</p></section>`;
 return `<section class="schema-example"><h2>Two chunks share one document</h2><p>An ingestion pipeline yields chunk <code>0</code>: <code>Notice period</code>, then chunk <code>1</code>: <code>Thirty days</code>. Both refer to document <code>firm-a/d1</code>. Its title and access are stored once.</p>${table('documents','Shared document metadata. No complete body string is required.',['tenant_id','document_id','title','public'],[['firm-a','d1','Lease notice','0']])}${table('document_access','One row per allowed user per document, shared by all its chunks.',['tenant_id','document_id','user_id'],[['firm-a','d1','reader-a']])}<p class="schema-link">chunks → documents and document_access through (tenant_id, document_id)</p>${table('chunks','Each source chunk has its own nonempty ID. SQLite also generates an internal integer key.',['id','tenant_id','document_id','chunk_id'],[['7','firm-a','d1','0'],['8','firm-a','d1','1']])}<p class="schema-link">chunks.id → chunk_pk in chunk_texts and term_chunks</p><div class="schema-children">${table('chunk_texts','Text is fetched after ranking, only for selected chunks.',['chunk_pk','text'],[['7','Notice period'],['8','Thirty days']])}${table('term_chunks','Each row links a term to a chunk and records its score contribution. These entries are conventionally called postings.',['tenant_id','term','chunk_pk','weight'],[['firm-a','lease','7','3'],['firm-a','notice','7','4'],['firm-a','period','7','1'],['firm-a','lease','8','3'],['firm-a','notice','8','3'],['firm-a','thirty','8','1'],['firm-a','days','8','1']])}</div><p>For <code>lease period</code>, chunk <code>0</code> scores <code>3 + 1 = 4</code> and chunk <code>1</code> scores <code>3</code>. The query checks document access, ranks chunks and returns their source IDs. Title storage is shared; the toy scoring rule still applies title boosts to each chunk, so title-only matches may offer weak text evidence.</p></section>`;
}
...
function walkthroughView(lesson){
 return `<article class="lesson-walkthrough" data-content="walkthrough">${lessonOrientation(lesson)}${walkthroughOverview(lesson)}${schemaExample(lesson)}<section class="walk-example"><h2>${escapeHTML(lesson.orientation?.example||'Follow an example')}</h2>${lesson.worked_example?workedExample(lesson):`<ol class="learning-reasoning">${lesson.problem.reasoning.map(step=>`<li>${escapeHTML(step)}</li>`).join('')}</ol>`}</section><section class="walk-implementation"><h2>${lesson.kind==='discussion'?'What to cover in your design':lesson.kind==='quiz'?'How to reason through the check':'How to build your solution'}</h2><p class="exercise-contract">${escapeHTML(lesson.brief.rule)}</p>${implementationGuide(lesson)}</section>${lesson.practice_check?practiceCheck(lesson):''}${productionContext(lesson)}<div class="scenario-actions">${btn(practiceLabel(lesson)+' '+icon('arrow'),'phase','primary','data-phase="1"')}${btn('Open knowledge base','phase','ghost','data-phase="3"')}</div></article>`;
}
function knowledgeView(lesson){
 const guide=lesson.implementation;
 return `<article class="lesson-knowledge" data-content="knowledge"><header class="knowledge-heading"><h2>Knowledge base</h2></header><section class="knowledge-terms"><h3>What the terms mean</h3><dl>${lesson.brief.vocabulary.map(item=>`<div><dt>${escapeHTML(item.term)}</dt><dd>${escapeHTML(item.meaning)}</dd></div>`).join('')}</dl></section><section class="knowledge-choices"><h3>What changes the design?</h3>${guide.decisions.map(decision=>`<section class="decision-introduction"><p>${escapeHTML(decision.lead_in)}</p><details class="knowledge-section"><summary>${escapeHTML(decision.question)}</summary><dl>${decision.branches.map(branch=>`<div><dt>${escapeHTML(branch.answer)}</dt><dd>${escapeHTML(branch.action)}</dd></div>`).join('')}</dl></details></section>`).join('')}</section>${guide.ranking_code?`<details class="knowledge-section"><summary>How chunk ranking produces an order</summary><pre class="teaching-code" tabindex="0" aria-label="Python ranking example"><code>${escapeHTML(guide.ranking_code)}</code></pre><p>${escapeHTML(guide.ranking_note)}</p></details>`:''}${lesson.concept?`<details class="knowledge-section"><summary>Python & implementation notes</summary><div class="markdown">${markdown(lesson.concept)}${markdown(lesson.after_example)}</div></details>`:''}${guide.connections?.length?`<section class="knowledge-related"><h3>Where to learn the connected steps</h3>${guide.connections.map(link=>`<div><a href="#lesson/${escapeHTML(link.id)}" data-action="lesson" data-id="${escapeHTML(link.id)}">${escapeHTML(findLesson(link.id).title)}</a><p>${escapeHTML(link.text)}</p></div>`).join('')}</section>`:''}${lesson.strategy.references?.length||guide.ranking_reference?`<section class="knowledge-related"><h3>Technical reading</h3>${[...(lesson.strategy.references||[]),...(guide.ranking_reference?[guide.ranking_reference]:[])].map(ref=>`<a href="${escapeHTML(ref.url)}" target="_blank" rel="noopener noreferrer">${escapeHTML(ref.title)} ↗</a>`).join('')}</section>`:''}${lesson.kind==='code'?btn('Models, full contract & tests','references','secondary'):''}</article>`;
}
function implementationGuide(lesson){
 const guide=lesson.implementation;
 if(!guide)return '';
 return `<div class="implementation-guide"><h3>${lesson.kind==='discussion'?'Which component owns each job?':'What to keep track of'}</h3><p>${escapeHTML(guide.state)}</p><h3>${lesson.kind==='discussion'?'How requests and failures move through the system':'Steps to implement'}</h3><ol class="learning-reasoning">${guide.steps.map(step=>`<li>${escapeHTML(step)}</li>`).join('')}</ol>${guide.code?`<h3>Python example</h3><pre class="teaching-code" tabindex="0" aria-label="Python selection example"><code>${escapeHTML(guide.code)}</code></pre>`:''}${guide.trace?`<div class="teaching-trace" role="region" aria-label="${escapeHTML(guide.trace.caption||'Budget trace')}" tabindex="0"><table><caption>${escapeHTML(guide.trace.caption||'Track the budget through the loop (tokens)')}</caption><thead><tr>${guide.trace.headers.map(header=>`<th scope="col">${escapeHTML(header)}</th>`).join('')}</tr></thead><tbody>${guide.trace.rows.map(row=>`<tr>${row.map(cell=>`<td>${escapeHTML(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`:''}</div>`;
}
