...
function schemaExample(lesson){
 ...
 return `<section class="schema-example"><h2>See how the four tables connect</h2><p>One synthetic document: tenant <code>firm-a</code>, ID <code>d1</code>, title <code>Lease notice</code>, body <code>Notice period</code>. It is private and shared with <code>reader-a</code>. Suppose SQLite assigns it ID <code>7</code>.</p>${table('units','One row identifies the searchable document or chunk. An empty chunk_id means the whole document.',['id','tenant_id','document_id','chunk_id','title','public'],[['7','firm-a','d1','""','Lease notice','0']])}<p class="schema-link">units.id = 7 → unit_id = 7 in each related table below</p><div class="schema-children">${table('contents','One body per unit. Search can score postings before fetching this text.',['unit_id','text'],[['7','Notice period']])}${table('grants','Zero or more allowed users per unit.',['unit_id','user_id'],[['7','reader-a']])}${table('postings','One row per distinct word in the unit. Title membership adds 3; body membership adds 1.',['tenant_id','term','unit_id','weight'],[['firm-a','lease','7','3'],['firm-a','notice','7','4'],['firm-a','period','7','1']])}</div><p>All five related rows point back to unit <code>7</code>. A query for <code>lease period</code> reads weights <code>3 + 1 = 4</code>, then uses the unit ID to recover the document metadata. The builder saves these relationships; the query follows them.</p></section>`;
}
